...
//     ${'$'+id} = $myGrid_${id};
    ...
    function createRowMenu() {
       ...
       if('${menuCode!}' === 'file_call'){
       	    rowMenu.menu('appendItem', {
	            text: '文件详细',
	            name: 'fileDetail',
	            iconCls: 'icon-diskmultiple',
	            onclick: function () {
	            	var url = "/web/memberCenter/batchExcelView";
    				window.open(url+'?fileDir='+rowMenu.rowData.file_addr+"/"+rowMenu.rowData.file_name);
	            }
	        });
	        rowMenu.menu('appendItem', {
	            text: '日志目录',
	            ...
	            iconCls: 'icon-diskmultiple',
	            onclick: function () {
	            	console.log(rowMenu.rowData);
	                //window.open('/common/downloadJdlj?fileDir='+rowMenu.rowData.file_addr+"/"+rowMenu.rowData.file_name);
	                loadDialog($myGrid_${id}, '日志目录', '/logs/getLog?fileDir='+rowMenu.rowData.file_addr+"/"+rowMenu.rowData.log_addr);
	            }
	        });
        }
       if('${menuCode!}' === 'list_job_log'){
	        rowMenu.menu('appendItem', {
	            text: '日志详细',
	            name: 'logDetail',
	            iconCls: 'icon-diskmultiple',
	            onclick: function () {
	            	var url = "/web/memberCenter/batchExcelView";
    				window.open(url+'?fileDir='+rowMenu.rowData.log_file);
	            }
	        });
        }
